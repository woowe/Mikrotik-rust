...
impl<'a> ApiRos<'a> {
	...
	fn new(s: &'a mut TcpStream) -> ApiRos<'a> {
		ApiRos {
			stream: s
		}
	}
	...
	fn write_sentence(&mut self, words: Vec<String>) -> u32{
		let mut ret: u32 = 0;
		for w in words {
			self.write_word(w);
			ret += 1;
	...
	fn read_sentence(&mut self) -> Vec<String>  {
	...
	fn login(&mut self, username: String, pwd: String) {
